...
def mutate_tradition(seq, pairs, v, v_list, T, distribution, position_to_group_map):
...
def parse_distribution_file(filepath):
    """
    Parses an output file from the RNA design program and extracts the
    ...
    Returns:
        dict: A dictionary representing the final distribution.
              Example: {(0,): {'A': 0.9973, 'C': 0.0007, 'G': 0.0020}, ...}
    """
    final_distribution = {}
    in_final_dist_section = False
    ...
    try:
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if "Final Distribution" in line:
                    in_final_dist_section = True
                    continue

                if not in_final_dist_section or not line or ':' not in line:
                    continue

                if in_final_dist_section and not (line.startswith('(') or line[0].isdigit()):
                    break
                # --- Parse the line ---
                try:
                    pos_str, probs_str = line.split(':', 1)

                    # 1. Parse the position group into a tuple of integers
                    pos_str_cleaned = pos_str.replace('(', '').replace(')', '')
                    position_key = tuple(int(p) for p in pos_str_cleaned.split(','))

                    # 2. Parse the probabilities into a dictionary
                    probabilities = {}
                    prob_pairs = probs_str.strip().split(',')
                    for pair in prob_pairs:
                        pair = pair.strip()
                        # Use regex to handle potential extra spaces
                        match = re.match(r'([A-Z]+)\s+([0-9\.]+)', pair)
                        if match:
                            choice, prob_val = match.groups()
                            probabilities[choice] = float(prob_val)

                    if probabilities:
                        final_distribution[position_key] = probabilities
...
def find_position_group(position, distribution):
    """Searches the distribution to find which group a nucleotide position belongs to."""
    # Check for single-nucleotide groups first (most common)
    if (position,) in distribution:
        return (position,)

    # If not found, search through all multi-nucleotide groups
    for pos_group in distribution.keys():
        if len(pos_group) > 1 and position in pos_group:
            return pos_group

    return None  # Return None if not found in any group


def create_position_to_group_map(distribution):
    """
    Creates a fast lookup map from a single nucleotide position to its full
    coupled group (e.g., mismatch, trimismatch).

    Args:
        distribution (dict): The distribution loaded from a SamplingDesign file.

    Returns:
        dict: A map where key is an int (position) and value is a tuple (the group).
    """
    pos_to_group = {}
    for group_key in distribution.keys():
        for position in group_key:
            pos_to_group[position] = group_key
    return pos_to_group


def preprocess_distribution(distribution):
    """
    Converts the loaded distribution into a more efficient format for repeated use
    by pre-calculating choices and normalized probability arrays.
    """
    processed_dist = {}
    for group, probs_dict in distribution.items():
        choices = list(probs_dict.keys())
        probabilities = np.array(list(probs_dict.values()))

        # Normalize probabilities once and store them
        probabilities /= probabilities.sum()

        processed_dist[group] = (choices, probabilities)
    return processed_dist


# structured mutation
def mutate_structured(seq, pairs, v, v_list, T, distribution, position_to_group_map):
    v_list = [v / T for v in v_list]
    probs = np.exp(v_list) / sum(np.exp(v_list))
    index = np.random.choice(list(range(len(seq))), p=probs)
    pairs_mt = []
    ...
    for pos_pair in pairs_mt:
        # The key for the distribution map must be a sorted tuple
        pos_group_key = tuple(sorted(pos_pair))

        # Check if this position group exists in the loaded distribution
        if pos_group_key in distribution:
            # Directly retrieve the pre-calculated choices and probabilities
            choices, probabilities = distribution[pos_group_key]

            cumulative_probabilities = np.cumsum(probabilities)
            random_numbers = np.random.rand(1)
            indices = np.searchsorted(cumulative_probabilities, random_numbers)
            new_pair = choices[indices[0]]

            # Apply the mutation to the sequence list
            nuc_list[pos_group_key[0]] = new_pair[0]
            nuc_list[pos_group_key[1]] = new_pair[1]
        else:
            # This is a fallback in case the pair isn't in the distribution
            new_pair = random.choice(['AU', 'UA', 'GC', 'CG', 'UG', 'GU'])
            nuc_list[pos_pair[0]] = new_pair[0]
            nuc_list[pos_pair[1]] = new_pair[1]

    # 4. Mutate UNPAIRED positions using the SamplingDesign distribution
    for pos_unpair in unpairs_mt:
        # Use the helper function to find the full group
        # full_pos_group = find_position_group(pos_unpair, distribution)
        full_pos_group = position_to_group_map.get(pos_unpair)
        if full_pos_group:
            # 'distribution' now holds the pre-processed data
            if full_pos_group in distribution:
                # Directly retrieve the pre-calculated choices and probabilities
                choices, probabilities = distribution[full_pos_group]
                cumulative_probabilities = np.cumsum(probabilities)
                random_numbers = np.random.rand(1)
                indices = np.searchsorted(cumulative_probabilities, random_numbers)
                new_choice_str = choices[indices[0]]

                if len(new_choice_str) == len(full_pos_group):
                    for i, position_in_group in enumerate(full_pos_group):
                        nuc_list[position_in_group] = new_choice_str[i]
                else:
                    pass
                    print("  - Error: Mismatch in length. No mutation applied.")
            else:
                # Fallback in case the group key isn't in the processed distribution
                print(
                    f"  - Warning: Position group {full_pos_group} not in distribution. Defaulting to random mutation.")
                if len(full_pos_group) == 1:
                    nuc_list[full_pos_group[0]] = random.choice(['A', 'C', 'G', 'U'])

        else:
            # Fallback: The position was not found in any group in the distribution.
            nuc_list[pos_unpair] = random.choice(['A', 'C', 'G', 'U'])

    e_time = time.time()
    return "".join(nuc_list)


def samfeo(id, target, f, steps, k, seed, t=1, check_mfe=True, sm=True, freq_print=FREQ_PRINT):
    start_time = time.time()

    path = f"D:/ether/results/eterna100_targeted_time/{id}.txt"
    distribution = parse_distribution_file(path)
    # Pre-compute the map ONCE before the loop starts
    processed_distribution = preprocess_distribution(distribution)

    position_map = create_position_to_group_map(distribution)
    print("pathhhhhhhhhhhhhhhhhhhhhhh = ", path)


    np.random.seed(seed)
    print(f'seed: {seed} for id: {id}')
    # ---------------
    ...
    for i in range(steps):
        ...
        seq_next = mutate(p.seq, pairs, p.v, p.v_list, t, processed_distribution, position_map)
        num_repeat = 0
        ...
        if (i + 1) % freq_print == 0:
            ...
            if check_mfe:
                print(
                    f"iter: {i + 1: 5d}\t value: {v_min: .4f}\t mfe count: {len(mfe_list): 5d}\t umfe count: {count_umfe}\t best iter: {iter_min} improve: {improve:.2e}, time: {time.time() - ss} , history {len(history)}")
            else:
                print(f"iter: {i + 1: 5d}\t value: {v_min: .4f}\t best iter: {iter_min} improve: {improve:.2e} time: ",
                      time.time() - ss)
...
# RNA design in batch
def design(path_txt, name, func, num_step, k, t, check_mfe, sm, base_seed):

    targets = []
    with open(path_txt) as f:
        for line in f:
            parts = line.strip().split()
            # Store a tuple of (id, structure)
            targets.append((parts[0], parts[1]))
    data = []
    for puzzle_id, target in targets:
        puzzle_name = puzzle_id  # Use the ID from the file
    cols = (
    ...
    for i, target in enumerate(targets):
        ...
        puzzle_name = target[0]

        target = target[1]
        puzzle_seed = base_seed + i

        start_time = time.time()
        k_best, log, mfe_list, umfe_list, dist_best, ned_best, ddg_best, elapsed_time = samfeo(puzzle_name, target,
                                                                                               func, num_step, k=k, t=t, seed=puzzle_seed,
                                                                                               check_mfe=check_mfe,
...
def design_para(path_txt, name, func, num_step, k, t, check_mfe, sm):
    ...
    for i_batch in range(0, len(targets), BATCH_SIZE):
        ...
        for j, result in enumerate(results_pool):
            ...
            puzzle_name, target = targets[idx_puzzle]
            target = targets[idx_puzzle]
            print("para target : ", target)
...
def run_repetition(i, cli_args, f_obj, name_input_str):


    global seed_np, name_pair, suffix

    seed_np = 2020 + (i + cli_args.start) * 2021
    np.random.seed(seed_np)

    name_pair = cli_args.init

    suffix = f"{i + cli_args.start}"

    print(f"[Core {i%cli_args.worker_count}] STARTING Repetition {i+cli_args.start} (Seed: {seed_np}, Suffix: {suffix})")
    ...
    try:
        design(
            ...
            k=cli_args.k,
            t=cli_args.t,
            check_mfe=not cli_args.nomfe,
            sm=not cli_args.nosm,
            base_seed=seed_np
        )
        print(f"[Core {i%cli_args.worker_count}] FINISHED Repetition {i+cli_args.start}")
        return (i + cli_args.start, "SUCCESS")
    except Exception as e:
        print(f"[Core {i%cli_args.worker_count}] FAILED Repetition {i+cli_args.start}: {e}")
        return (i + cli_args.start, "FAILED")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--worker_count", type=int, default=7)
    ...
    global name_pair, stop, seed_np, suffix
    name_pair = args.init
    ...
    if args.online:
        ...
        for line in sys.stdin:
            ...
            print(target)
            start_time = time.time()
            k_best, log, mfe_list, umfe_list, dist_list, ned_best, ddg_best, elapsed_time = samfeo(target, f_obj,
                                                                                                   args.step, k=args.k,
                                                                                                   t=args.t,
                                                                                                   check_mfe=not args.nomfe,
                                                                                                   sm=not args.nosm)  # rna and ensemble defect
    ...
    for i in range(args.repeat):
        ...
        if args.para:
            ...
            if max_cores < 1:
                max_cores = 1
            num_workers = min(args.worker_count, max_cores, args.repeat)  # Cannot have more workers than tasks

            print(f"\n--- Running {args.repeat} repetitions in PARALLEL on {num_workers} cores ---")


            task_indices = range(args.repeat)
            ...
            task_func = functools.partial(
                ...
                cli_args=args,
                f_obj=f_obj,
                name_input_str=name_input
            )

            start_total_time = time.time()
            with multiprocessing.Pool(processes=num_workers) as pool:

                results = pool.map(task_func, task_indices)

            end_total_time = time.time()
            print(f"\n--- All parallel repetitions complete in {end_total_time - start_total_time:.2f}s ---")
            print("Run summary:")
            for res in results:
                print(f"  Repetition {res[0]}: {res[1]}")
